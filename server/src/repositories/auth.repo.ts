...
export async function setLastSignedInAt(userId: Types.ObjectId): Promise<void> {
await connectDB();
await User.updateOne({ _id: userId }, { $set: { lastSignedInAt: new Date() } }).exec();
}
...
replace tokenHash with new hash (old token becomes invalid immediately)
