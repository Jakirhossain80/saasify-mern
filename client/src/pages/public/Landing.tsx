    ...
    <div className="text-slate-900">
        ...
        <div className="mx-auto max-w-[1680px] px-4 sm:px-6 lg:px-8 pt-14 pb-16 md:pt-20 md:pb-24 lg:pt-28 lg:pb-32">
          <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
            <div className="space-y-8">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-600/10 border border-blue-600/20 text-blue-700 text-xs font-semibold uppercase tracking-wider">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-600 opacity-30" />
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-slate-900 leading-[1.1] tracking-tight">
                Multi-tenant <span className="text-blue-600">Projects</span> Dashboard for Teams
              </h1>
                ...
                {user ? (
                  <>
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-60"
                    >
            ...
            <div className="relative">
              <div className="rounded-2xl shadow-2xl overflow-hidden border border-slate-800 bg-slate-900">
      ...
      <section id="features" className="border-t border-slate-200 bg-white">
        <div className="mx-auto max-w-[1680px] px-4 sm:px-6 lg:px-8 py-16 md:py-20">
          <div className="max-w-3xl mb-10 md:mb-14">
            <div className="text-blue-600 font-semibold text-xs uppercase tracking-widest mb-3">Enterprise Features</div>
      ...
      <section className="border-t border-slate-200 bg-slate-50">
        <div className="mx-auto max-w-[1680px] px-4 sm:px-6 lg:px-8 py-16 md:py-20">
          ...
          <div className="mt-12 md:mt-16 rounded-2xl border border-slate-200 bg-white shadow-sm px-5 py-4 flex flex-wrap items-center justify-center gap-3 sm:gap-6">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-widest">Quick Access:</span>

            <Link
              to="/platform"
              className="inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold text-slate-700 hover:text-blue-600 hover:bg-slate-50 transition-colors"
            >
      ...
      <section id="pricing" className="border-t border-slate-200 bg-white">
        <div className="mx-auto max-w-[1680px] px-4 sm:px-6 lg:px-8 py-16 md:py-20">
          <div className="text-center max-w-3xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight">Simple pricing</h2>
            <p className="mt-4 text-slate-600 text-base leading-relaxed">
...
function FeatureCard({ title, desc }: { title: string; desc: string }) {
  return (
    <div className="p-6 rounded-2xl border border-slate-200 bg-white hover:border-blue-600/20 hover:shadow-xl hover:shadow-blue-600/5 transition-all">
      <div className="text-lg font-bold text-slate-900">{title}</div>
      <div className="mt-3 text-sm text-slate-600 leading-relaxed">{desc}</div>
    </div>
  );
}
...
function StepCard({ step, title, desc }: { step: string; title: string; desc: string }) {
  return (
    <div className="relative text-center group">
      <div className="w-16 h-16 bg-white rounded-2xl shadow-sm border border-slate-200 flex items-center justify-center mx-auto mb-6 font-extrabold text-2xl text-blue-600 group-hover:scale-110 transition-transform">
        {step}
      </div>
      <div className="text-xl font-bold mb-3 text-slate-900">{title}</div>
      <div className="text-slate-600 text-sm leading-relaxed max-w-sm mx-auto">{desc}</div>
    </div>
  );
}
...
function SecurityCard({ title, desc }: { title: string; desc: string }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="w-10 h-10 rounded-full bg-green-100 text-green-600 flex items-center justify-center mb-5">
  ...
  return (
    <div
      className={[
        ...
        featured
          ? "border-2 border-blue-600 bg-blue-600/[0.02] shadow-xl shadow-blue-600/10 scale-[1.02]"
          : "border-slate-200",
      ].join(" ")}
    >
      ...
      <ul className="mt-5 space-y-3 text-sm font-medium text-slate-700 flex-1">
        {items.map((it) => (
          <li key={it} className="flex items-start gap-3">
            <Check className="h-5 w-5 mt-0.5 text-blue-600" />
            <span>{it}</span>
          </li>
        ))}
      </ul>
